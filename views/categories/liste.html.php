        ...
        <div class="bg-white p-10 rounded-lg shadow-lg max-w-2xl w-full overflow-hidden mb-5 mt-5">
            ...
            <form class="mb-6" action="<?=WEBROOT?>" method="post">
                <div class="flex items-center">
                    <input name="nomCategorie" id="inputTypeCategorie" class="block w-full border border-gray-300 rounded-md shadow-sm py-2 px-3 focus:outline-none focus:ring-[#a6c1ee] focus:border-[#a6c1ee] sm:text-sm <?= add_class_invalid("nomCategorie")?>" placeholder="Nom du nouveau categorie">
            ...
            <div class="overflow-x-auto max-h-[300px] overflow-y-auto">
                <table class="min-w-full bg-white">
                    <thead class="bg-[#eea6af] text-white">
                        <tr>
                            <th class="px-4 py-2 border-b">Nom du categorie</th>
                            <th class="px-4 py-2 border-b">Actions</th>
                        </tr>
                    </thead>
                    <tbody>
                        <?php foreach($reponse['data'] as $categorie):?>
                ...
                <div class="flex justify-center mt-6">
                    <nav class="inline-flex space-x-2">
                        <a href="<?=WEBROOT?>?action=liste-categorie&controller=categorie&page=<?php if ($currentPage == 0) echo 0; else echo $currentPage-1?>" class="inline-flex items-center justify-center w-8 h-8 text-gray-700 bg-white border border-gray-300 rounded-full hover:bg-gray-200 transition duration-300">
                            <i class="fas fa-chevron-left"></i>
                        </a>
                        <?php for ($i=0; $i < $reponse['pages'] ; $i++) :?>
                            <a href="<?=WEBROOT?>?action=liste-categorie&controller=categorie&page=<?=$i?>" class="inline-flex items-center justify-center w-8 h-8 rounded-full hover:bg-gray-200 transition duration-300<?php if ($i == $currentPage) echo"text-white bg-[#eea6af] border border-[#eea6af]";else echo "text-gray-700 bg-white border border-gray-" ?>"><?= $i+1?></a>
                        <?php endfor ?>
                        <a href="<?=WEBROOT?>?action=liste-categorie&controller=categorie&page=<?php if ($currentPage == $reponse['pages']-1) echo $reponse['pages']-1; else echo $currentPage+1?>" class="inline-flex items-center justify-center w-8 h-8 text-gray-700 bg-white border border-gray-300 rounded-full hover:bg-gray-200 transition duration-300">
