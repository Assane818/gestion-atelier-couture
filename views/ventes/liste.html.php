  ...
  <div class="container mx-auto flex items-center justify-center h-full p-6">
    <div class="bg-white p-10 rounded-lg shadow-lg max-w-4xl w-full overflow-hidden">
      <div class="flex justify-between items-center mb-6">
        <h1 class="text-2xl font-bold text-[#eea6af]">Liste des Ventes</h1>
        <button class="bg-[#eea6af] text-white px-5 py-2 rounded-full hover:bg-[#c2c9fb] transition duration-300">
          <a href="<?= WEBROOT ?>?action=form-vente&controller=vente">Ajouter</a>
        </button>
      </div>
      <form class="mb-6" action="<?=WEBROOT?>" method="get">
        <div class="flex gap-4">
          ...
          <div>
            <label for="article" class="block text-sm font-medium text-gray-700">Filtrer par client</label>
            <select id="client" name="clientId" class="mt-1 block w-full border border-[#c2c9fb] rounded-md shadow-sm py-2 px-3 focus:outline-none focus:ring-2 focus:ring-[#fbc2eb] focus:border-transparent sm:text-sm bg-white transition duration-300">
              <option value="">Sélectionnez un client</option>
              <?php foreach ($clients as $client) : ?>
                <option value="<?= $client['clientId'] ?>" <?php if (isset($_REQUEST['clientId']) && $client['clientId'] == $_REQUEST['clientId']) echo 'selected'?>><?= $client['prenom'].' '.$client['nom'] ?></option>
              <?php endforeach; ?>
            </select>
          </div>
          <div class="flex items-end">
            <input type="hidden" name="action" value="listeFiltre-vente">
        ...
        <table class="min-w-full bg-white border">
          <thead class="bg-[#eea6af] text-white">
            <tr>
              ...
              <th class="py-2 px-4 border-b min-w-[100px]">Client</th>
          ...
          <tbody>
            <?php foreach ($reponse['data'] as $vente) :
              ...
              <tr class="bg-gray-100 hover:bg-gray-200 transition duration-300">
                ...
                <td class="py-2 px-4 border-b text-center"><?= $vente['montant'] ?></td>
                <td class="py-2 px-4 border-b text-center"><?= $vente['prenom'].' '.$vente['nom'] ?></td>
      ...
      <?php if ($reponse['pages'] > 1): ?>
        <div class="flex justify-center mt-6">
          <nav class="inline-flex space-x-2">
            <a href="<?=WEBROOT?>?action=listeFiltre-vente&controller=vente&dateFiltre=<?php if (isset($_GET['dateFiltre'])) echo $_GET['dateFiltre']?>&articleId=<?php if (isset($_GET['articleId'])) echo $_GET['articleId']?>&clientId=<?php if (isset($_GET['fourId'])) echo $_GET['clientId']?>&page=<?php if ($currentPage == 0) echo 0; else echo $currentPage-1?>" class="inline-flex items-center justify-center w-8 h-8 text-gray-700 bg-white border border-gray-300 rounded-full hover:bg-gray-200 transition duration-300">
              <i class="fas fa-chevron-left"></i>
            </a>
            <?php for ($i=0; $i < $reponse['pages'] ; $i++) :?>
              <a href="<?=WEBROOT?>?action=listeFiltre-vente&controller=vente&dateFiltre=<?php if (isset($_GET['dateFiltre'])) echo $_GET['dateFiltre']?>&articleId=<?php if (isset($_GET['articleId'])) echo $_GET['articleId']?>&clientId=<?php if (isset($_GET['clientId'])) echo $_GET['clientId']?>&page=<?=$i?>" class="inline-flex items-center justify-center w-8 h-8 rounded-full hover:bg-gray-200 transition duration-300<?php if ($i == $currentPage) echo"text-white bg-[#eea6af] border border-[#eea6af]";else echo "text-gray-700 bg-white border border-gray-" ?>"><?= $i+1?></a>
            <?php endfor ?>
            <a href="<?=WEBROOT?>?action=llisteFiltre-vente&controller=vente&dateFiltre=<?php if (isset($_GET['dateFiltre'])) echo $_GET['dateFiltre']?>&articleId=<?php if (isset($_GET['articleId'])) echo $_GET['articleId']?>&clientId=<?php if (isset($_GET['clientId'])) echo $_GET['clientId']?>&page=<?php if ($currentPage == $reponse['pages']-1) echo $reponse['pages']-1; else echo $currentPage+1?>" class="inline-flex items-center justify-center w-8 h-8 text-gray-700 bg-white border border-gray-300 rounded-full hover:bg-gray-200 transition duration-300">
