    ...
    <div class="container mx-auto flex items-center justify-center h-full p-6">
        <div class="bg-white p-10 rounded-lg shadow-lg max-w-4xl w-full overflow-hidden">
            <div class="flex justify-between items-center mb-6">
                <h1 class="text-2xl font-bold text-[#eea6af]">Détail de l'approvisionnement</h1>
            </div>
            <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                <div class="bg-white p-4 rounded-lg shadow-md">
                    <h2 class="text-xl font-semibold mb-2 text-[#eea6af]">Informations de l'utilisateur</h2>
                    <p><strong>Nom:</strong> <?= $reponse['appro']['nomComplet'] ?></p>
                    <p><strong>Email:</strong> <?= $reponse['appro']['login'] ?></p>
                    <p><strong>Role:</strong> <?= $reponse['appro']['name'] ?></p>
                </div>
                <div class="bg-white p-4 rounded-lg shadow-md">
                    <h2 class="text-xl font-semibold mb-2 text-[#eea6af]">Informations du Fournisseur</h2>
                    <p><strong>Nom:</strong> <?= $reponse['appro']['nomFour'] ?></p>
                    <p><strong>Téléphone:</strong> <?= $reponse['appro']['telFour'] ?></p>
                    <p><strong>Adresse:</strong> <?= $reponse['appro']['adresseFour'] ?></p>
                </div>
                <div class="bg-white p-4 rounded-lg shadow-md col-span-2">
                    <p><strong>Date:</strong> <?= $reponse['appro']['date'] ?></p>
                    <p><strong>Montant:</strong> <span class="text-green-600"><?= $reponse['appro']['montant'] ?> FCFA</span></p>
                </div>
            </div>
                    ...
                    <?php foreach ($reponse['detail'] as $article) : ?>
                        <div class="min-w-[250px] bg-white p-4 rounded-lg shadow-lg hover:shadow-xl transition duration-300">
                            ...
                            <div class="flex justify-between">
                                ...
                                <span class="text-green-600"><?= $article['prixAppro'] * $article['qteAppro'] ?> FCFA</span>
                            </div>
                        </div>
                    <?php endforeach;?>
                </div>
            ...
            <div class="mt-7 text-center mb-2">
                <a href="<?= WEBROOT ?>?action=liste-appro&controller=appro&page=0" class="bg-[#eea6af] text-white w-full px-5 py-2 mb-1 rounded-full hover:bg-[#c2c9fb] transition duration-300">Retour</a>
            </div>
        </div>
    </div>
</div>
