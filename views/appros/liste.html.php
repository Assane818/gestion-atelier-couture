  ...
  <div class="container mx-auto flex items-center justify-center h-full p-6">
    <div class="bg-white p-10 rounded-lg shadow-lg max-w-4xl w-full overflow-hidden">
      <div class="flex justify-between items-center mb-6">
        <h1 class="text-2xl font-bold text-[#eea6af]">Liste des Approvisionnement</h1>
        <button class="bg-[#eea6af] text-white px-5 py-2 rounded-full hover:bg-[#c2c9fb] transition duration-300">
          <a href="<?= WEBROOT ?>?action=form-appro&controller=appro">Ajouter</a>
        </button>
      </div>
      <form class="mb-6" action="<?=WEBROOT?>" method="get">
        <div class="flex gap-4">
          ...
          <div>
            <label for="article" class="block text-sm font-medium text-gray-700">Filtrer par Fournisseur</label>
            <select id="fornisseur" name="fourId" class="mt-1 block w-full border border-[#c2c9fb] rounded-md shadow-sm py-2 px-3 focus:outline-none focus:ring-2 focus:ring-[#fbc2eb] focus:border-transparent sm:text-sm bg-white transition duration-300">
              <option value="">Sélectionnez un fournisseur</option>
              <?php foreach ($fournisseurs as $fournisseur) : ?>
                <option value="<?= $fournisseur['fourId'] ?>" <?php if (isset($_REQUEST['fourId']) && $fournisseur['fourId'] == $_REQUEST['fourId']) echo 'selected'?>><?= $fournisseur['nomFour'] ?></option>
              <?php endforeach; ?>
            </select>
          </div>
          <div class="flex items-end">
            <input type="hidden" name="action" value="listeFiltre-appro">
        ...
        <table class="min-w-full bg-white border">
          <thead class="bg-[#eea6af] text-white">
            <tr>
              ...
              <th class="py-2 px-4 border-b min-w-[100px]">Fournisseur</th>
              <th class="py-2 px-4 border-b min-w-[100px]">Telephone</th>
          ...
          <tbody>
            <?php foreach ($reponse['data'] as $appro) :
              ...
              <tr class="bg-gray-100 hover:bg-gray-200 transition duration-300">
                ...
                <td class="py-2 px-4 border-b text-center"><?= $appro['montant'] ?></td>
                <td class="py-2 px-4 border-b text-center"><?= $appro['nomFour'] ?></td>
                <td class="py-2 px-4 border-b text-center"><?= $appro['telFour'] ?></td>
                <td class="py-2 px-4 border-b flex items-center justify-center gap-2">
                  <form action="<?= WEBROOT ?>" method="post">
                    <input type="hidden" name="action" value="detail-appro">
                    <input type="hidden" name="approId" value="<?= $appro['approId'] ?>">
                    <input type="hidden" name="controller" value="appro">
      ...
      <?php if ($reponse['pages'] > 1): ?>
        <div class="flex justify-center mt-6">
          <nav class="inline-flex space-x-2">
            <a href="<?=WEBROOT?>?action=listeFiltre-appro&controller=appro&dateFiltre=<?php if (isset($_GET['dateFiltre'])) echo $_GET['dateFiltre']?>&articleId=<?php if (isset($_GET['articleId'])) echo $_GET['articleId']?>&fourId=<?php if (isset($_GET['fourId'])) echo $_GET['fourId']?>&page=<?php if ($currentPage == 0) echo 0; else echo $currentPage-1?>" class="inline-flex items-center justify-center w-8 h-8 text-gray-700 bg-white border border-gray-300 rounded-full hover:bg-gray-200 transition duration-300">
              <i class="fas fa-chevron-left"></i>
            </a>
            <?php for ($i=0; $i < $reponse['pages'] ; $i++) :?>
              <a href="<?=WEBROOT?>?action=listeFiltre-appro&controller=appro&dateFiltre=<?php if (isset($_GET['dateFiltre'])) echo $_GET['dateFiltre']?>&articleId=<?php if (isset($_GET['articleId'])) echo $_GET['articleId']?>&fourId=<?php if (isset($_GET['fourId'])) echo $_GET['fourId']?>&page=<?=$i?>" class="inline-flex items-center justify-center w-8 h-8 rounded-full hover:bg-gray-200 transition duration-300<?php if ($i == $currentPage) echo"text-white bg-[#eea6af] border border-[#eea6af]";else echo "text-gray-700 bg-white border border-gray-" ?>"><?= $i+1?></a>
            <?php endfor ?>
            <a href="<?=WEBROOT?>?action=listeFiltre-appro&controller=appro&dateFiltre=<?php if (isset($_GET['dateFiltre'])) echo $_GET['dateFiltre']?>&articleId=<?php if (isset($_GET['articleId'])) echo $_GET['articleId']?>&fourId=<?php if (isset($_GET['fourId'])) echo $_GET['fourId']?>&page=<?php if ($currentPage == $reponse['pages']-1) echo $reponse['pages']-1; else echo $currentPage+1?>" class="inline-flex items-center justify-center w-8 h-8 text-gray-700 bg-white border border-gray-300 rounded-full hover:bg-gray-200 transition duration-300">
