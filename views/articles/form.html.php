      ...
      <div class="bg-white p-10 rounded-lg shadow-lg max-w-2xl w-full overflow-auto mb-5 mt-5">
        ...
        <form enctype="multipart/form-data" class="space-y-6" action="<?=WEBROOT?>" method="post" enctype="multipart/form-data">
          ...
          <div>
            ...
            <input name="qteStock" id="inputQuantite" class="mt-1 block w-full border border-[#c2c9fb] rounded-md shadow-sm py-2 px-3 focus:outline-none focus:ring-[#a6c1ee] focus:border-[#a6c1ee] sm:text-sm <?= add_class_invalid("qteStock")?>" placeholder="Quantité en stock">
            <div id="errorQuantite" class="text-red-500 text-sm mt-1"><?=$erros["qteStock"]??""?></div>
          </div>
          <div>
            <label for="prix" class="block text-sm font-medium text-gray-700">Prix</label>
            <input type="text" id="inputPrix" name="prixAppro" class="mt-1 block w-full border border-[#c2c9fb] rounded-md shadow-sm py-2 px-3 focus:outline-none focus:ring-[#a6c1ee] focus:border-[#a6c1ee] sm:text-sm <?= add_class_invalid("prixAppro")?>" placeholder="Prix de l'article">
          ...
          <div class="flex justify-end">
            ...
            <button type="button" class="text-[#eea6af] px-5 py-2 rounded-full hover:text-[#c2c9fb] border border-[#eea6af] hover:border-[#c2c9fb] mr-4 transition duration-300"><a href="<?=WEBROOT?>?action=liste-article&controller=article&page=0">Annuler</a></button>
            <button type="submit" class="bg-[#eea6af] text-white px-5 py-2 rounded-full hover:bg-[#c2c9fb] transition duration-300" name="btnSave" id="addArticle">Ajouter</button>
          </div>
        </form>
      </div>
    </div>
