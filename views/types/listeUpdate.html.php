        ...
        <div class="bg-white p-10 rounded-lg shadow-lg max-w-2xl w-full overflow-hidden mb-5 mt-5">
            ...
            <form class="mb-6" action="<?= WEBROOT ?>" method="post">
                <div class="flex items-center">
                    <input name="nomType" id="inputTypeCategorie" class="block w-full border border-[#c2c9fb] rounded-md shadow-sm py-2 px-3 focus:outline-none focus:ring-[#a6c1ee] focus:border-[#a6c1ee] sm:text-sm <?= add_class_invalid("nomType")?>" placeholder="Nom du nouveau type" value="<?=$type['nomType']?>">
                    <div id="errorTypeCategorie" class="text-red-500 text-sm mt-1"><?=$erros["nomType"]??""?></div>
                    <input type="hidden" name="action" value="update-type" id="upadteType">
                    <input type="hidden" name="typeId" id="typeId" value="<?=$type['typeId']?>">
                    <input type="hidden" name="controller" value="type">
                    <button type="submit" class="ml-4 bg-[#eea6af] text-white px-5 py-2 rounded-full hover:bg-[#c2c9fb] transition duration-300" name="btnUpdate" id="addTypeCategorie">Modifier</button>
                </div>
            </form>
